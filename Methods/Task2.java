...
//        Зачем - разберемся, когда будем изучать I/O Streams
        scanner.close();
        doChoice(a, b); // вводим метод doChoice для сравнения 2 чисел
        double result = Math.pow(a, b); //возведение в степень
...
//                Каст к int необязателен, но позволяет избежать выведения в консоль дробной части
    ...
    static void doChoice(int a, int b) { //метод doChoice для сравнения 2 чисел, которому на вход поступают 2 числа
        if (a % 2 == 0 || b % 3 == 0) //условие
            System.out.println(a == b); //вывод на экран
        return; //возврат значения, чтобы можно было дальше производить операции над числами
    }
}
